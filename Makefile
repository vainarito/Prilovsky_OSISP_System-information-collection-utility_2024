...
BLKID_INCLUDE = -I/usr/include/blkid
BLKID_LIBS = -lblkid
...
$(TARGET): $(OBJ)
	$(CC) $(FLAGS) $(GTK_INCLUDE) $(BLKID_INCLUDE) $(OBJ) -o $(RELEASE)/$(TARGET) -lncursesw -lncurses -lform -lusb-1.0 $(GTK_LIBS) $(BLKID_LIBS)

$(OUT_OBJ)%.o: $(PREF_SRC)%.c
	$(CC) $(FLAGS) $(GTK_INCLUDE) $(BLKID_INCLUDE) -c $< -o $@
